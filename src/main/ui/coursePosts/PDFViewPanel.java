...
public class PDFViewPanel extends JPanel { 
	...
	public PDFViewPanel(Course course) {							//TODO: constructor with parameters: byte array file
		...
		byte[] pdfBytes = null;
		for( Object i : course.getPdfFiles().keySet()) {
			pdfBytes = course.getPdfFiles().get(i); //bos.toByteArray();
		}
	...
	public PDFViewPanel() {							//TODO: constructor with parameters: byte array file
		...
		//file to be displayed
		File file = new File("res/course6.pdf");
        FileInputStream fis;
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
		try {
			fis = new FileInputStream(file);
